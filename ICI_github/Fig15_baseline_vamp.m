...
for s0=1:length(sinr_w)
    ...
    %%
    H_p_delay=mean(abs(H_delay_obs(:)).^2);
    %%       
    Hopt_delay_w=zeros(M,n_delay,NSlots);
    for tau0=1:NSlots
        Ct_int=Ct_int_w(:,:,tau0);
        H_obs=H_delay_obs(:,:,tau0); %% M,n_delay
        %%
        if mod(tau0,100)==0
            fprintf('t0=%d\n',tau0);
        end
        ...
        H_p_BG=max(real(H_p_delay-trace(Cu)/M),1e-5);       
        H_e = vamp_baseline_ici(H_obs,Cu,0.1,H_p_BG/0.1,100);
        %%
        %%
...
grid on
%%
save('major_online_baseline_vamp_CDLA_varysinr_inr_3.mat','sinr_w','nmse_opt');