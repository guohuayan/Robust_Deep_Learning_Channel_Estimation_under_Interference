function xest = vamp_baseline_ici(Y,Cw,lambda,BG_var,niter)
...
%     [x_BG_pos,x_BG_var_tmp,x_BG_nzp,x_BG_var] = vamp_BG_Gauss(BG_mu,BG_var,x_ext,x_var_ext,lambda);
    ...
    x_pir=reshape(x_pir_vec,[M,n_delay]);
    
	%%
    xest=reshape(x_BG_pos,[M,n_delay]);
    err=x_old-xest;
    err=sum(abs(err(:)).^2);
    if err<1e-3
        break
...
function [mu_pos,var_pos] = LMMSE_module(Y,Cw,x_pir,x_var_pir)
    ...
    mu_pos=zeros(M,n_delay);
    Y_obs=Y-x_pir;
    for n0=1:n_delay
        H_obs=Y_obs(:,n0);
%         Ch=x_var_pir*eye(M);       
        mu_pos(:,n0)=Cov/Cw*H_obs;
    end
    mu_pos=mu_pos+x_pir;
    var_pos=trace(Cov)/M;
    %%
