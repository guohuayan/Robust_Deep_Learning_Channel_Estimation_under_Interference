...
function [ f, g ] = pseudoLF(params, S_HAT, N, T)
    beta = params(1);
    alpha = params(2);
    ...
    f = sum(sum(- S_HAT.*(Z) + log(cosh(Z)))); % -log pseudo likelihood


    g = [sum(sum(tanh(Z).*hvZ - S_HAT.*hvZ )); sum(sum(-tanh(Z) + S_HAT))]; % Gradient

%     H = NaN*ones(2); % Hessian
