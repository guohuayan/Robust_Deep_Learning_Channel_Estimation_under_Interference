# compare to v2 change to loss function back to non-linear
import os
...
def train(net,trainloader, criterion,optimizer, Fm,Fa,Cs,power_d,snr_w,Filter_w,device = torch.device('cpu')):
    ...
    Filter_w=Filter_w.to(device)
    # Fpower_d=Fpower_d.to(device)
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        # snr=torch.rand(batch_len,device=device)*torch.tensor(20,device=device)-torch.tensor(15,device=device)
        # # snr=torch.rand(1,device=device)*torch.tensor(30,device=device)-torch.tensor(20,device=device)
        # noise_var=torch.float_power(torch.tensor(10.0,device=device), -snr/torch.tensor(10.0,device=device))
        # noise_var=noise_var.cfloat()        
        # noise_vfreq=noise_var.reshape(H_freq.shape[0],1,1).expand(H_freq.shape)
        # noise=torch.randn(H_freq.shape,dtype=torch.complex64,device=device) 
        # noise=noise*torch.sqrt(noise_vfreq)
        # H_LS=H_freq+noise
        # H_LS=H_LS.to(device, dtype=torch.complex64)
        # H_delay_LS=torch.matmul(conFm,H_LS) # 1000 15 64
        # h_p=Fpower_d.reshape(1,H_delay.shape[1],1).expand(H_delay.shape)
        # noise_vdelay=noise_var.reshape(H_delay.shape[0],1,1).expand(H_delay.shape)
        # noise_vdelay=noise_vdelay.to(device, dtype=torch.complex64)
        # h_p=h_p/(h_p+noise_vdelay)
        # H_vbi=h_p*H_delay_LS

        
        H_vbi=torch.matmul(H_vbi,conFa)
        err=torch.square( torch.abs(H_vbi - H_delay)).mean(dim=(1,2))

        nmse_vec=NMSE_element(H_vbi,H_delay)
        

        outputs, z, b = net(H_vbi,err)

        loss, sp_thr, avg_ratio=criterion(outputs,H_delay,z, b,nmse_vec,device=device)
        pr_w+=sp_thr
        ratio_w+=avg_ratio
    ...
    pr_w = pr_w /normalizer_train
    ratio_w = ratio_w /normalizer_train
    return total_epoch_loss_train, pr_w, ratio_w

def train_criterion(output, obs, z, b,nmse_vec,device = torch.device('cpu')):
    weight=torch.tensor(1.0,device=device)/nmse_vec
    ...
    mse = (torch.div(mse1,mse2)*weight)
    avg_ratio=mse.mean()

    mse_clip = torch.min(mse,torch.tensor(1.5,device=device))
    # mse_clip = mse_clip/torch.tensor(0.5,device=device)
    # loss_weights = torch.exp(mse_clip)
    loss_weights = torch.exp(torch.square(mse_clip))
    # loss_weights = torch.exp(torch.square(mse_clip))/torch.exp(torch.tensor(2,device=device))
    
    # mse_clip = mse_clip/mse_clip.mean()
    ...
    p_x=torch.square(torch.abs(obs)) # 1000 96 64
    Pobs = torch.square(obs_sp).sum(dim=(3)) # 1000 96 64
    # x_thr=Pobs.mean(dim=(1,2))/torch.tensor(10,device=device)
    # x_thr=Pobs.mean(dim=(1,2))*nmse_vec
    x_thr=Pobs.mean(dim=(1,2))*nmse_vec*torch.tensor(0.5)
    x_thr2=x_thr.reshape(x_thr.shape[0],1,1).expand(-1,p_x.shape[1],p_x.shape[2])
    sp_x=((p_x>x_thr2)*1.0).mean(dim=[1,2])

    # sp_x=sp_x/torch.tensor(1.2,device=device)

    sp_x=torch.max(sp_x,torch.tensor(1/40,device=device))
    sp_x=torch.min(sp_x,torch.tensor(1/10,device=device))
    sp_thr=sp_x.mean()
    ...
    # loss = 200*mse+loss_reg # since exp(1)=2.7183, the weight for mse becomes 200/2=100
    # return loss, sp_thr
    loss = torch.tensor(1.0,device=device)*mse+torch.tensor(1.0,device=device)*loss_reg
    return loss, sp_thr, avg_ratio
...
if __name__ == '__main__':
    ...
    # PATH_train='./major/save_data/major_channel_cdl_off_pre_cnn_1e5.pt'
    ...
    conFa=torch.conj(Fa.permute(1,0))
    # FCs=torch.load(PATH_train)['Cs']
    # Fpower_d=torch.load(PATH_train)['power_d']
    ...
    # PATH_filter = './major/save_result/LMSE_filter_rddlay_snr_n20_10.pt'
    PATH_filter = './major/save_result/LMSE_filter_rddlay_snr_n20_3.pt'
    ...
    # Model_PATH = "./major/major_cdlmodel_rddelay_noici_cbn_Filtervbi_para_n20_10_cdlA_v3.pt"
    # Model_PATH = "./major/major_cdlmodel_rddelay_noici_cbn_Filtervbi_para_n20_3_cdlA_v3.pt"
    Model_PATH = "./major/major_cdlmodel_rddelay_noici_cbn_Filtervbi_para_n20_3_cdlA_v3_meanall.pt"
    ...
    for epoch in range(NUM_EPOCHS):
        total_epoch_loss_train, sp_w, ratio_w = train(vae,trainloader, train_criterion,optimizer, Fm,Fa,
                                FCs,Fpower_d,snr_w,Filter_w,device=device)
        
        print("[epoch %03d]  average training loss: %.4f" % (epoch, total_epoch_loss_train))
        print("[epoch %03d]  training sparse threshold: %.4f" % (epoch, sp_w))
        print("[epoch %03d]  training ratio: %.4f" % (epoch, ratio_w))
        ...
        if epoch % TEST_FREQUENCY == 0:
            ...
            # test_loss=torch.max(torch.tensor([tloss_0/err0,tloss_1/err1, tloss_2/err2,tloss_3/err3,
            ...
            #     tloss_4/err4, tloss_5/err5, tloss_6/err6,tloss_7/err7,tloss_8/err8]))
